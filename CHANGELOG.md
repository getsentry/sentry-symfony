...
### Changed
 - Make the `RequestListener` more extendable by making all members at least `protected` (#176, thanks @bouland)
...
### Changed
 ...
 - Re-sort the constructor's arguments of `RequestListener`
 - The `SentrySymfonyClient` is no longer an optional argument of `RequestListener`; it's now required
### Fixed
